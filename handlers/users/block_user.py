...
from keyboards.default import back, start_markup, block_confirm
from keyboards.default.admis_keyboards import user_notfound
from loader import dp
from states import Blocking, Starting

from datetime import datetime, timedelta



@dp.message_handler(text='/block', user_id=admins, state=Starting.choose)
async def f_command_block(message: types.Message):
    await message.answer(text='Введите user-ID', reply_markup=back)
    await Blocking.user_id.set()


@dp.message_handler(user_id=admins, state=Blocking.user_id)
    ...
    if text == 'Назад':
        ...
        if user:
            await message.answer(text='Пользователь найден:\n'
                                 ...
                                 reply_markup=block_confirm)
            await state.update_data(
                {
                    'user': user[0]
                }
            )
            await Blocking.confirm.set()
        else:
            await message.answer(text='Нет такого пользователя', reply_markup=user_notfound)
            await Blocking.user_notfound.set()


@dp.message_handler(user_id=admins, state=Blocking.user_notfound)
...
@dp.message_handler(user_id=admins, state=Blocking.confirm)
async def f_user_notfound(message: types.Message, state: FSMContext):
    text = message.text
    ...
    if text == 'Заблокировать':
        ...
        await message.answer(text='Выберите действие из предложенных', reply_markup=block_confirm)
